...
('STL001', 'Starlink Device #001', 'Starlink Standard', 'New York, USA', 'available', 15.00, 200, 20, 99.50),
('STL002', 'Starlink Device #002', 'Starlink Standard', 'Los Angeles, USA', 'available', 15.00, 200, 20, 99.20),
('STL003', 'Starlink Device #003', 'Starlink Premium', 'London, UK', 'available', 25.00, 300, 30, 99.80),
('STL004', 'Starlink Device #004', 'Starlink Standard', 'Toronto, Canada', 'available', 18.00, 200, 20, 99.30),
('STL005', 'Starlink Device #005', 'Starlink Premium', 'Sydney, Australia', 'available', 22.00, 300, 30, 99.60);
...
('plisio_api_key', '', 'string', 'payments', 'Plisio.net API key'),
...
CREATE TRIGGER `trigger_auto_referral_code` BEFORE INSERT ON `users` FOR EACH ROW BEGIN
    IF NEW.referral_code IS NULL OR NEW.referral_code = '' THEN
        SET NEW.referral_code = UPPER(SUBSTRING(MD5(CONCAT(NEW.username, UNIX_TIMESTAMP(), RAND())), 1, 10));
    END IF;
END$$

-- Trigger to create referral relationships
CREATE TRIGGER `trigger_create_referral_relationships` AFTER INSERT ON `users` FOR EACH ROW BEGIN
    DECLARE referrer_id INT;
    DECLARE level2_referrer_id INT;
    DECLARE level3_referrer_id INT;
    ...
    IF NEW.referred_by IS NOT NULL THEN
        ...
        -- Level 2 referral
        SELECT referred_by INTO level2_referrer_id FROM users WHERE id = NEW.referred_by;
        IF level2_referrer_id IS NOT NULL THEN
            INSERT INTO referrals (referrer_id, referred_id, level, commission_rate, status)
            VALUES (level2_referrer_id, NEW.id, 2, 5.00, 'active');
            
            -- Level 3 referral
            SELECT referred_by INTO level3_referrer_id FROM users WHERE id = level2_referrer_id;
            IF level3_referrer_id IS NOT NULL THEN
                INSERT INTO referrals (referrer_id, referred_id, level, commission_rate, status)
                VALUES (level3_referrer_id, NEW.id, 3, 3.00, 'active');
            END IF;
        END IF;
    END IF;
END$$
